...
plot(f, main= "F tests for structural break for hedge")
...
intraGroupVar <-movavg(apply(meanBasket, 1, var),20)
...
#SIGNIFICANT COEFFICIENTS IN VAR

library(vars)
signVAR <-c()
tempDates<-c()
windowLength<-50
for (i in c(1:(nrow(meanBasket)-windowLength))) {
  tempVAR<-VAR(meanBasket[i:(i+windowLength)], type=c("const"), lag.max=6) 
  #meanBasket.VAR<-VAR(meanBasket, type=c("const"))
  #summary(meanBasket.VAR)
  #causality(meanBasket.VAR )
  #
  tempSum = 0
  for (colName in names(meanBasket)) {
    #colName <-"bankmeans.ts"
    tempRes <- summary(tempVAR$varresult[[colName]])$coefficients
    tempRes <-tempRes[row.names(tempRes)!="const", 4]
    
    tempSum = tempSum + sum(tempRes<0.05)
    
  }
  tempDates <-c(tempDates,index(meanBasket)[i+windowLength] ) 
  signVAR[[i]] = tempSum
}
plot(as.Date(tempDates), signVAR, xlab = 'dates', type="l", ylab='number of significant variables')


#pairwise granger second approach
install.packages("car")
library(car)

startIndex = 9073 #change this line
...
rownames(grangerDF)<-colnames(merged)[2:ncol(merged)]
colnames(grangerDF)<-colnames(merged)[2:ncol(merged)]
for (i in 2:(ncol(merged)-1)){
  for (j in (i+1):ncol(merged)) {
    if(i!=j) {
      x<-merged[1:nrow(merged), i]
      y<-merged[1:nrow(merged), j]
      
      colNames <-names(merged)[c(i,j)]
      p1<-names(merged)[i]
      p2<-names(merged)[j]
      granger <- VAR(merged[, colNames], type=c("const"), lag.max=6)
      mod<-granger$varresult
      mod1<-granger$varresult[[p1]]
      coefs<-names(coef(mod1))
      grangerDF[i-1,j-1] <- linearHypothesis(mod1, coefs[grep(p2, coefs)])$`Pr(>F)`[2]
      
      mod2<-granger$varresult[[p2]]
      coefs<-names(coef(mod2))
      grangerDF[j-1,i-1] <- linearHypothesis(mod2, coefs[grep(p1, coefs)])$`Pr(>F)`[2]
...
#set.seed(567)
#LO = layout_with_fr(G_Grouped) #comment this line out to use the same layout
plot.igraph(g1,  vertex.color = rainbow(4, alpha=0.4)[groupVector], layout=LO, vertex.label=NA, vertex.size=5,arrow.size=0.01, arrow.width=0.1, curved=TRUE, edge.arrow.size=0.3)
legend('topleft', c('broker', 'insurer', 'bank', 'hedge fund'), lty=1, col=rainbow(4, alpha=0.4), bty='n', cex=.75, pch=15)
#write.csv( grangerDF, "temp/graph06.csv")
saveLayout <- LO
...
#calculate rolling amount of connections
res<-c()
dates<-c()
for (startIndex in seq(1, 36*12*12, 36*6)){
  endIndex = startIndex+35
  brokers_shared <- df[startIndex:endIndex, c(1,2:26)]
  ins_shared <- df[startIndex:endIndex, c(1,27:51)]
  banks_shared <- df[startIndex:endIndex, c(1,52:76)]
  ...
  res<-c(res, sum(grangerDF<0.05, na.rm=TRUE))
  dates<-c(dates, as.Date(df[endIndex, 1]))
...
plot(as.Date(dates), res, type="l", xlab = "dates", yaxt='n')

install.packages('svars')
library(svars)

m<-VAR(meanBasket, , lag.max=12,type=('c') )
